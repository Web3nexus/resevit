...
class TwoFactorChallenge extends Page implements HasForms
{
    ...
    protected static string $view = 'filament.dashboard.pages.auth.two-factor-challenge';
    ...
    public function mount(): void
    {
        if (!session()->has('login.id')) {
            redirect()->route('filament.dashboard.auth.login');
        }
    ...
    public function authenticate(): void
    {
        ...
        if ($isValid) {
            ...
            $this->redirect(config('filament.dashboard.home_url', '/dashboard'));
        } else {
            Notification::make()
                ->title('Invalid code')
                ->danger()
