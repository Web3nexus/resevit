...
class TwoFactorChallenge extends Page implements HasForms
{
    ...
    protected static string $view = 'filament.customer.pages.auth.two-factor-challenge';
    ...
    public function mount(): void
    {
        if (!session()->has('login.id')) {
            redirect()->route('filament.customer.auth.login');
        }
    ...
    public function authenticate(): void
    {
        ...
        if ($isValid) {
            ...
            $this->redirect(config('filament.customer.home_url', '/customer'));
        } else {
            Notification::make()
                ->title('Invalid code')
                ->danger()
